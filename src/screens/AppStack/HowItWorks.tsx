...
const HowItWorks: React.FC = () => {
  ...
  const handleProceed = async () => {
    if (agreed) {
      try {
        ...
        navigation.replace(SCREENS.DashBoard); // or your default screen
      } catch (error) {
        Alert.alert(
          "Error",
  ...
  const CustomCheckBox = ({ title, checked, onPress }) => {
    return (
      ...
      >
        <Image
          source={checked ? IMAGE.Check : IMAGE.UnCheck}
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.headerView}>
        ...
        >
          <Image source={IMAGE.TranslateImage} style={styles.translateIcon} />
        </TouchableOpacity>
      </View>
